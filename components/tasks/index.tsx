...
const commonInputValue: inputWithError = {
  value: '',
  error: false,
  message: '',
};
...
const Tasks = () => {
  ...
  const [projectName, setProjectName] = useState(
    commonInputValue as inputWithError
  );
  ...
  const columns = [
    ...
    },
    {
      title: 'Task Edit',
      key: 'task-edit',
      render: (data: tastItem) => (
        <Button key={'edit-button'} onClick={() => handleEdit(data)}>
          Edit
        </Button>
      ),
    },
  ...
  const handleEditTask = () => {
    if (
      ...
      setTaskList((prev) => {
        let task: number = [...prev].findIndex(
          (task: tastItem) => task.id === SelectedTaskForEdit.id
        );
  ...
  const handleFormChange = (
    ...
    switch (name) {
      case 'taskName':
        return setTaskName((prev) => ({ ...prev, ...commonInputValue, value }));
      case 'projectName':
        return setProjectName((prev) => ({
          ...prev,
          ...commonInputValue,
          value,
        }));
  ...
  return (
    <div className="task-container">
      <Button
        icon={<PlusCircleOutlined color="green" />}
      ...
      >
        ...
        >
          <CommonInput
            label="Task Name"
            {...destructureInputState(taskName)}
          ...
          <CommonSelect
            label="Profile Name"
            options={projectList}
            {...destructureInputState(projectName)}
            onChange={handleFormChange}
            name="projectName"
            required
